...
const Navbar: React.FC = () => {
  return (
    <nav className="fixed w-full z-50 bg-[#050505]/95 backdrop-blur-xl border-b border-[#D4C5A5]/10 shadow-2xl transition-all duration-300">
      <div className="w-full px-4 lg:px-6 xl:px-12 h-16 flex items-center justify-between">
        <div
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="flex items-center gap-3 lg:gap-4 group cursor-pointer h-full py-2 shrink-0"
        >
          {/* Logo Compacto */}
          <div className="h-full aspect-square p-1.5 bg-black/20 rounded border border-[#D4C5A5]/30 shadow-[0_0_15px_rgba(212,197,165,0.1)] group-hover:border-[#D4C5A5]/60 transition-all duration-500">
        ...
        <div className="hidden lg:flex items-center gap-4 xl:gap-8 ml-auto">
          ...
          {[
            ...
            <a
              key={link.label}
              className="relative text-[10px] lg:text-xs xl:text-sm uppercase tracking-[0.15em] font-bold text-white/70 hover:text-white transition-colors duration-300 group/link whitespace-nowrap"
              href={link.href}
            >
              {link.label}
          ...
          <a
            ...
            className="ml-2 bg-[#D4C5A5] hover:bg-[#C3B494] text-[#050505] px-4 py-2 rounded-[1px] text-[10px] lg:text-xs uppercase tracking-[0.2em] font-black transition-all hover:scale-105 hover:shadow-[0_0_20px_rgba(212,197,165,0.3)] whitespace-nowrap"
          >
            Consulta
          </a>
        </div>
        ...
        <div className="lg:hidden">
          <span className="material-symbols-outlined text-[#D4C5A5] text-3xl">menu</span>
        </div>
      </div>
    </nav>
  );
